...
//Handler que verifica si hay decrementos de valor asignado en el bucle
public class DecrementoValorAsignadoHandler implements Handler {
	...
	public boolean handle(String codigoFuente) {
		if (comprobarExpresion(codigoFuente)) {
			if (nextHandler != null) {
				System.out.println("Decremento de valor asignado encontrado");
				return nextHandler.handle(codigoFuente);
			} else {
				throw new UnsupportedOperationException("No tengo siguiente Handler");
			}
		}
		System.out.println("No hay decrementos valor asignado en el bucle");
		return nextHandler.handle(codigoFuente);
	}

	//Función auxiliar que verifica si hay decrementos de valor asignado en el código fuente
